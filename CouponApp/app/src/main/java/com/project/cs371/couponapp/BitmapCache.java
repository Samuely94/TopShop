...
public class BitmapCache {
    ...
    private BitmapCache() {
        if( this.cacheSize == 0 ) {
            Log.d(SearchActivity.AppName, "Yikes, BitmapCache.cacheSize not set!");
            return;
        }
        if( maxW == 0 || maxH == 0 ) {
            Log.d(SearchActivity.AppName, "Yikes, set maxW and maxH to get bitmap scaling.");
        }
    ...
    public synchronized void setBitmap(String key, Bitmap bitmap) {
        if (mMemoryCache.get(key) == null) {
            ...
            if( bitmap == null ){
                Log.d(SearchActivity.AppName, "Yikes, bitmap too big " + key);
    ...
    public synchronized Bitmap getBitmap(String key) {
        ...
        if( bm == null ) {
            Log.d(SearchActivity.AppName, "BMCache miss "+key);
        } else {
            Log.d(SearchActivity.AppName, "BMCache hit " + key);
        }
        return bm;
    }
}
